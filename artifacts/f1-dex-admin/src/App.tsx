...
import {
  ...
  Menu, Pencil, Plus, Radio, RefreshCw, Search, Server, Settings2, ShieldCheck,
...
const navItems = [
  ...
  { href: '/drivers', label: 'Driver cards', icon: Trophy },
...
type DriverFormState = { name: string; shortName: string; number: string; team: string; nationality: string; rarity: string; rating: string; active: boolean; imageUrl: string };
const emptyDriver: DriverFormState = { name: '', shortName: '', number: '', team: '', nationality: '', rarity: 'common', rating: '75', active: true, imageUrl: '' };
function DriverModal({ driver, close, onSaved }: { driver?: Driver; close: () => void; onSaved: (message: string) => void }) {
  const [form, setForm] = useState<DriverFormState>(driver ? { name: driver.name, shortName: driver.shortName, number: String(driver.number), team: driver.team, nationality: driver.nationality, rarity: driver.rarity, rating: String(driver.rating), active: driver.active, imageUrl: driver.imageUrl ?? '' } : emptyDriver);
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!form.name.trim() || !form.shortName.trim() || !form.team.trim() || !form.nationality.trim()) return;
    const payload = { name: form.name.trim(), shortName: form.shortName.trim(), number: Number(form.number) || 0, team: form.team.trim(), nationality: form.nationality.trim(), rarity: form.rarity as DriverInput['rarity'], rating: Math.max(1, Math.min(100, Number(form.rating) || 1)), active: form.active, imageUrl: form.imageUrl.trim() || null };
  ...
  return <div className="fixed inset-0 z-40 flex items-end justify-center bg-[#20242d]/60 p-0 backdrop-blur-sm sm:items-center sm:p-4"><div role="dialog" aria-modal="true" className="max-h-[94dvh] w-full max-w-2xl overflow-y-auto rounded-t-2xl border border-border bg-card p-5 shadow-2xl sm:rounded-2xl sm:p-7"><div className="mb-6 flex items-start justify-between"><div><div className="mono-font text-[10px] uppercase tracking-[.18em] text-[#d61f45]">{driver ? 'Edit entry' : 'New entry'}</div><h2 className="display-font text-3xl font-extrabold uppercase">{driver ? driver.name : 'Add driver card'}</h2></div><button data-testid="button-close-driver-modal" onClick={close} className="rounded-lg p-2 text-muted-foreground transition hover:bg-muted hover:text-foreground"><X className="h-5 w-5" /></button></div>
    <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">{[['name', 'Full name'], ['shortName', 'Short name'], ['number', 'Car number'], ['team', 'Team'], ['nationality', 'Nationality'], ['rating', 'Rating / 100'], ['imageUrl', 'Image URL']].map(([key, label]) => <label key={key} className={key === 'imageUrl' ? 'sm:col-span-2' : ''}><span className="mb-1.5 block text-xs font-bold text-foreground">{label}</span><input data-testid={`input-driver-${key}`} type={key === 'number' || key === 'rating' ? 'number' : 'text'} value={form[key as keyof DriverFormState] as string} onChange={e => change(key as keyof DriverFormState, e.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm outline-none transition focus:border-[#d61f45] focus:ring-2 focus:ring-[#d61f45]/15" required={key !== 'imageUrl'} /></label>)}
      <label><span className="mb-1.5 block text-xs font-bold text-foreground">Rarity</span><select data-testid="select-driver-rarity" value={form.rarity} onChange={e => change('rarity', e.target.value)} className="h-10 w-full rounded-lg border border-input bg-background px-3 text-sm outline-none focus:border-[#d61f45]">{Object.values(DriverInputRarity).map(value => <option key={value} value={value}>{value[0].toUpperCase() + value.slice(1)}</option>)}</select></label>
      <label className="flex items-center gap-3 self-end pb-2"><button type="button" data-testid="button-toggle-driver-active" onClick={() => change('active', !form.active)} className={cn('relative h-6 w-11 rounded-full transition', form.active ? 'bg-emerald-500' : 'bg-muted-foreground/30')}><span className={cn('absolute top-1 h-4 w-4 rounded-full bg-white transition-transform', form.active ? 'translate-x-6' : 'translate-x-1')} /></button><span className="text-xs font-bold">Available in drops</span></label>
      <div className="mt-2 flex justify-end gap-3 border-t border-border pt-5 sm:col-span-2"><button type="button" data-testid="button-cancel-driver" onClick={close} className="rounded-lg px-4 py-2.5 text-xs font-bold text-muted-foreground hover:bg-muted">Cancel</button><button data-testid="button-submit-driver" disabled={pending} className="flex items-center gap-2 rounded-lg bg-[#d61f45] px-5 py-2.5 text-xs font-bold text-white transition hover:bg-[#b61939] disabled:opacity-60">{pending && <RefreshCw className="h-3.5 w-3.5 animate-spin" />}{driver ? 'Save changes' : 'Add to card pool'}</button></div>
    </form></div></div>;
}
...
function DriversPage() {
  ...
  return <Shell title="Driver cards" eyebrow="Collection / catalog" action={<button data-testid="button-add-driver-header" onClick={() => setModal('new')} className="flex items-center gap-2 rounded-lg bg-[#d61f45] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-[#b61939]"><Plus className="h-4 w-4" /> <span className="hidden sm:inline">Add driver</span></button>}>
    ...
    <div className="mb-6 flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm text-muted-foreground">Tune the pool that keeps every community drop interesting.</p></div><label className="relative w-full sm:w-72"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input data-testid="input-search-drivers" type="search" placeholder="Search name, team, nationality" value={search} onChange={e => setSearch(e.target.value)} className="h-10 w-full rounded-lg border border-input bg-card pl-9 pr-3 text-sm outline-none transition focus:border-[#d61f45]" /></label></div>
    {query.isLoading ? <LoadingPanel rows={6} /> : query.isError ? <ErrorPanel retry={() => query.refetch()} /> : !query.data?.length ? <EmptyPanel title={search ? 'No matching cards' : 'Card pool is empty'} detail={search ? 'Try another name, team, or nationality.' : 'Add the first driver to start building the collection.'} action={!search && <button data-testid="button-add-driver-empty" onClick={() => setModal('new')} className="rounded-lg bg-[#d61f45] px-4 py-2 text-xs font-bold text-white">Add driver</button>} /> : <div className="overflow-hidden rounded-xl border border-border bg-card"><div className="hidden grid-cols-[1.5fr_1fr_.7fr_.6fr_.7fr_90px] gap-4 border-b border-border bg-muted/60 px-5 py-3 mono-font text-[9px] uppercase tracking-[.16em] text-muted-foreground md:grid"><span>Driver</span><span>Team</span><span>Rarity</span><span>Rating</span><span>Status</span><span /></div>{query.data.map(driver => <div key={driver.id} data-testid={`row-driver-${driver.id}`} className="grid gap-3 border-b border-border p-4 last:border-0 transition hover:bg-muted/30 md:grid-cols-[1.5fr_1fr_.7fr_.6fr_.7fr_90px] md:items-center md:gap-4 md:px-5"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#20242d] text-white"><span className="display-font text-xl font-bold">{driver.number}</span></div><div className="min-w-0"><div className="truncate text-sm font-bold">{driver.name}</div><div className="mono-font mt-0.5 text-[10px] text-muted-foreground">{driver.shortName} · {driver.nationality}</div></div></div><div className="text-xs font-semibold text-muted-foreground md:text-foreground">{driver.team}</div><div><span className={cn('inline-flex rounded border px-2 py-1 text-[9px] font-bold uppercase tracking-wider', rarityTone(driver.rarity))}>{driver.rarity}</span></div><div className="display-font text-xl font-bold">{driver.rating}</div><div><StatusPill tone={driver.active ? 'good' : 'neutral'}>{driver.active ? 'Active' : 'Paused'}</StatusPill></div><div className="flex items-center gap-1 md:justify-end"><button data-testid={`button-edit-driver-${driver.id}`} onClick={() => setModal(driver)} className="rounded-md p-2 text-muted-foreground transition hover:bg-[#d61f45]/10 hover:text-[#d61f45]" aria-label={`Edit ${driver.name}`}><Pencil className="h-3.5 w-3.5" /></button><button data-testid={`button-delete-driver-${driver.id}`} disabled={del.isPending} onClick={() => remove(driver)} className="rounded-md p-2 text-muted-foreground transition hover:bg-red-50 hover:text-red-600" aria-label={`Delete ${driver.name}`}><Trash2 className="h-3.5 w-3.5" /></button></div></div>)}</div>}
...
function Router() {
  return <ErrorBoundary resetKey={useLocation()[0]}><Switch><Route path="/" component={Dashboard} /><Route path="/drivers" component={DriversPage} /><Route path="/players" component={PlayersPage} /><Route path="/activity" component={ActivityPage} /><Route path="/bot" component={BotPage} /><Route component={NotFound} /></Switch></ErrorBoundary>;
}
